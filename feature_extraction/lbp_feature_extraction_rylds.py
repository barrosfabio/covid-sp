...
train_directory = 'C:/Users/Fabio Barros/Git/covid-sp/Images/RYDLS-20'
train_directory_second_round = 'C:/Users/Fabio Barros/Git/covid-sp/data/covid_train_59_fase2/covid_sp_train_59.csv'
...
def verify_valid_img(path):
    ...
    is_valid_image = imghdr.what(path)
    is_jpg = path.find('jpg')
    if is_valid_image in possible_formats or is_jpg!=-1:
        return True
    else:
        return False
...
def create_feature_matrix_test(test_directory, lbp_extractor):
    ...
    for file in test_filelist:
        ...
        if verify_valid_img(file_path):
            _fileNumber = str(test_filelist.index(file)+1) + " from " + str(len(test_filelist))
            print("Processing: " + _fileNumber + " -- PATH: " + file_path)
...
matrix_train_second_round = pd.read_csv(train_directory_second_round)
print("Saving Training Feature Matrix to CSV")
print("Saving Train Feature Matrix to "+feature_matrix_train_path)
feature_matrix_train =  feature_matrix_train.append(matrix_train_second_round)
